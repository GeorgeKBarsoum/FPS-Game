...
namespace Graphics._3D_Models
{
    ...
    class md2LOL
    {
        ...
        public vec3 getMaxPos()
        {
            ...
            float x = mVertices[0].x;
            float y = mVertices[0].y;
            float z = mVertices[0].z;

            for (int i = 1; i<mVertices.Count; i++)
            {
                x = Math.Max(x, mVertices[i].x);
                y = Math.Max(y, mVertices[i].y);
                z = Math.Max(z, mVertices[i].z);
            }
            vec4 res = new vec4(1);
            res.x = x;
            res.y = y;
        ...
        public vec3 getMinPos()
        {
            ...
            for (int i = 1; i < mVertices.Count; i++)
            {
                x = Math.Min(x, mVertices[i].x);
                y = Math.Min(y, mVertices[i].y);
                z = Math.Min(z, mVertices[i].z);
            }
